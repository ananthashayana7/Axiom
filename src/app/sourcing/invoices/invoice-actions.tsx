    ...
    const handleOverrideRequest = async (requestType: 'place_hold' | 'clear_hold' | 'payment_reversal') => {
        const actionLabel = requestType === 'place_hold'
            ...
            : requestType === 'clear_hold'
                ...
                : 'reverse the payment';
        setOverrideDialog({ requestType, actionLabel, reason: '' });
    };

    const submitOverride = async () => {
        if (!overrideDialog || !overrideDialog.reason.trim()) return;
        const { requestType, reason } = overrideDialog;
            ...
            <Dialog open={!!overrideDialog} onOpenChange={(open) => { if (!open) setOverrideDialog(null); }}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Enter the reason to {overrideDialog?.actionLabel} for this invoice. This will be logged in the audit trail.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-2">
                        ...
                        <Textarea
                            ...
                            onChange={(e) => setOverrideDialog((prev) => prev ? { ...prev, reason: e.target.value } : null)}
                        />
                    </div>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setOverrideDialog(null)}>Cancel</Button>
                        <Button onClick={submitOverride} disabled={isLoading || !overrideDialog?.reason.trim()}>
